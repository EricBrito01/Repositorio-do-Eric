...
select *  from tb_venda;
   ...
   select *  from tb_venda inner join tb_carrinho on tb_carrinho.tb_carrinho_id = tb_venda.tb_carrinho_id 
   join tb_produto on tb_produto.tb_produto_id = tb_carrinho.tb_produto_id 
   where tb_venda_id = 2 and tb_carrinho_ativo=2;

alter table tb_cliente
add column tb_cliente_complemento varchar(128);


alter table tb_cliente
drop column tb_cliente_complemento;

select * from tb_cliente
;
select * from tb_cliente where tb_cliente_id =2;


select  * from tb_produto;

UPDATE tb_produto SET tb_produto_estoque=tb_produto_estoque - 2 where tb_produto_id= 1; 